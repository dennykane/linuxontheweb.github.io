...
/*Old
const OldDB = function(){//DB«

let db;
const check = (rv, nrows, iflog)=>{//«
	if (!(rv.rows||rv.message)) {
cwarn(rv);
cerr("Unknown return value");
	...
	if (!rv.message) {
		let rows = rv.rows;
		if (nrows && rows.length < nrows) return false;
		if (iflog) {
			for (let row of rows){
				log(row);
			}
...
const check_db_rv = check;

const create_all_tables=()=>{//«

return new Promise(async(Y,N)=>{

let rv;
rv = await this.createTable("Blobs",["value BLOB"]);
if (!check(rv)) return Y();
//rv = await this.createTable("Nodes",["type CHAR(1)", "parent INTEGER", "name VARCHAR(255)"],{primaryKey: true});
//The node value is either 1=directory, 2=link or an id for Blobs
rv = await this.createTable("Nodes",["value INTEGER", "parent INTEGER", "name VARCHAR(255)"],{primaryKey: true});
if (!check(rv)) return Y();
//rv = await this.createTable("Files",["blobId INTEGER"]);
//if (!check(rv)) return Y();
rv = await this.createTable("Links",["value VARCHAR(4096)"]);
if (!check(rv)) return Y();

rv = await this.insertRow("Nodes",["value","parent", "name"],[1, 0, DEF_BRANCH_NAME]);
if (!check(rv)) return Y();
let defid = rv.insertId;
if (defid !== 1) {
cerr(`Expected defid==1 (got ${defid})`);
	return;
}
rv = await this.createTable(`d${defid}`,[]);
if (!check(rv)) return Y();
Y(true);
...
this.init=(root, dbname, dbsize, branch_name)=>{//«
	return new Promise(async(Y,N)=>{
		if (db) return Y(true);
		db = openDatabase(dbname, "", dbname, dbsize);
		if (!root) return Y(true);
		let rv = await this.getListById("Nodes",[DEF_NODE_ID]);
		if (rv.code===5) {
			if (!await create_all_tables()) {
				return Y();
			}
		}
		else if (!rv.rows){
cerr("Unknown value returned");
log(rv);
			return Y();
		}
		root.id = DEF_NODE_ID;
...
this.listTables=()=>{//«
	...
	return new Promise((Y,N)=>{
		db.transaction(tx=>{ 
			let s = `SELECT name FROM sqlite_schema WHERE type ='table' AND name NOT LIKE 'sqlite_%'`;
			tx.executeSql(s,[],(tx, rv)=>{Y(rv);},(tx, err)=>{Y(err);});
		});
	});
}//»
this.createTable=(name, labels, opts={})=>{//«
	if (!db) return;
	return new Promise((Y,N)=>{
		db.transaction(tx=>{ 
			let primkey="";
			if (opts.primaryKey) primkey=" PRIMARY KEY";
			let s =`CREATE TABLE IF NOT EXISTS ${name} (id INTEGER${primkey}`;
			if (!labels.length) s+=")";
			else s+=`, ${labels.join(", ")})`;
			tx.executeSql(s,[],(tx, rv)=>{
				Y(rv);
			},(tx, err)=>{
				Y(err);
			});
...
this.insertRow=(name, labels, values)=>{//«
	if (!db) return;
	return new Promise((Y,N)=>{
		db.transaction(tx=>{ 
			let q="";
			for (let i in labels) q += (q == "" ? "" : ", ") + "?";
			let s =`INSERT INTO ${name} (${labels.join(", ")}) VALUES (${q})`;
			tx.executeSql(s,values,(tx, rv)=>{
				Y(rv); 
			},(tx, err)=>{
				Y(err); 
			});
...
this.getListById=(tabname, idarr)=>{//«
	if (!db) return;
	return new Promise((Y,N)=>{
		db.transaction(tx=>{ 
			let s =`SELECT * FROM ${tabname} WHERE ID in (${idarr.join(",")})`;
		   tx.executeSql(s,[],(tx, rv)=>{
				Y(rv);
			},(tx, err)=>{
				Y(err);
...
this.updateRow=(name, label, values)=>{//«
if (!db) return;
return new Promise((Y,N)=>{//«
	db.transaction(async tx=>{ 
		let s =`UPDATE ${name} SET ${label} = ? WHERE id = ?`;
		tx.executeSql(s,values,(tx, rv)=>{
			Y(rv);
		},(tx, err)=>{
cerr(err);
...
this.deleteRow=(tabname, id)=>{//«
//DELETE FROM table_name WHERE condition;
if (!db) return;
return new Promise((Y,N)=>{
	db.transaction(tx=>{ 
	let s =`DELETE FROM ${tabname} WHERE id=${id}`;
//log(s);
	   tx.executeSql(s,[],(tx, rv)=>{
	//log("DONE",tx, rv);
			Y(rv);
...
this.getNodeNames=(id, type)=>{//«

if (!db) return;
return new Promise((Y,N)=>{
	db.transaction(tx=>{ 
	let typs="";
	if (type) typs=` AND type="${type}"`;
	let s =`SELECT name FROM Nodes WHERE id=${id}${typs}`;
//log(s);
	   tx.executeSql(s,[],(tx, rv)=>{
	//log("DONE",tx, rv);
			Y(rv);
...
this.getNodeByNameAndParId=(name, parid)=>{//«
...
this.deleteDatabases=arr=>{//«

return new Promise(async(Y,N)=>{
for (let dbnm of arr){
cwarn(dbnm);
    let db = new DB();
    if (!await db.init(null, dbnm, DBSIZE)) {
log(`Skipping!`);
        continue;
    }   
    let all = await db.listTables();
    let rows = all.rows;
    if (!rows) continue;
    rows = Array.from(rows);
    for (let row of rows){
        let nm = row.name;
        if (nm.match(/^_/)) continue;
log(nm);
        let rv = await db.dropTable(nm);
log(rv);
    }
} 
Y();
...
this.moveNode=(id, fromId, toId, newName)=>{//«

//console.log("id",id,"from",fromId,"to",toId, "name",newName);
...
db.transaction(tx=>{ 
	if (fromId !== toId){
		let use_exe_cb;
		if(!newName) use_exe_cb= exe_cb;
		tx.executeSql(`DELETE FROM d${fromId} WHERE id=${id}`)
		tx.executeSql(`INSERT INTO d${toId} (id) VALUES (${id})`);
		tx.executeSql(`UPDATE Nodes SET parent = ${toId} WHERE id = ${id}`, [], use_exe_cb);
	}

	if (newName){
		tx.executeSql(`UPDATE Nodes SET name = ? WHERE id = ${id}`, [newName], exe_cb);
	}
...
this.removeNode=(id, parId)=>{//«
if (!db) return;
//log(`id=${id}, parId=${parId}`);

return new Promise(async(Y,N)=>{
	let exe_cb=(tx, rv)=>{Y(true);};
	db.transaction(tx=>{ 
		tx.executeSql(`DELETE FROM d${parId} WHERE id=${id}`)
		tx.executeSql(`UPDATE Nodes SET parent = 0 WHERE id = ${id}`, [], exe_cb);
	}, err=>{
		Y();
cerr(err.message);
	},()=>{
...
this.createNode=(name, value, parId)=>{//«

if (!db) return;
//log(`CreateNode: name=${name}, parId=${parId}`);
return new Promise(async(Y,N)=>{
	let id;
	let exe_cb=(tx, rv)=>{Y(id);};
	let rv = await this.insertRow("Nodes", ["value", "parent", "name"], [value , parId, name]);
	if (!check_db_rv(rv)) {
cerr("GEOPMTYJH");
Y();
return;
	...
	id = rv.insertId;
	db.transaction(tx=>{ 
//		tx.executeSql(`INSERT INTO Files (id, blobId) VALUES (${id}, 0)`);
		tx.executeSql(`INSERT INTO d${parId} (id) VALUES (${id})`, [], exe_cb);
	}, err=>{
		Y();
cerr(err.message);
	},()=>{
...
return new Promise((Y,N)=>{
	db.transaction(tx=>{ 
		tx.executeSql(`SELECT blobId FROM Files WHERE ID in (${id})`,[],(tx, rv)=>{
			if (!check(rv, 1)) return Y();
			tx.executeSql(`SELECT path from Blobs WHERE ID in (${rv.rows[0].blobId})`,[],(tx,rv)=>{
				if (!check(rv, 1)) return Y();
				Y(rv.rows[0].path);
			},(tx, err)=>{
				Y()
