...
let ext = Win.ext;
//let loader_base = '/node/three/examples/jsm/loaders';
			let use_loader;
			let loaderMod;
			if (ext=="glb") {
				const {GLTFLoader} = await import(`/node/three/examples/jsm/loaders/GLTFLoader.js`);
				loaderMod = GLTFLoader;
			}
//				use_loader = "GLTFLoader";
			else if (ext=="fbx") {
				const {FBXLoader} = await import(`/node/three/examples/jsm/loaders/FBXLoader.js`);
				loaderMod = FBXLoader;
//				use_loader = "FBXLoader";
			}
			let loader = new loaderMod();
			loader.loadArrayBuffer( buffer, obj=>{
//log(obj);
				model = obj.scene||obj;
//log(scene);
//log(obj);
...
//				model = obj.scene;  // model 3D object is loaded
//log(model);
//				let anim0 = obj.animations[0];
