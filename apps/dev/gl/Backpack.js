...
import { Renderer, Camera, Transform, Orbit, Program, GLTFLoader, GLTFSkin, Vec3, Color, TextureLoader } from '/node/ogl/src/index.mjs';
...
const controls = new Orbit(camera,{element: Main});
...
const lutTexture = TextureLoader.load(gl, {
	src: '/node/ogl/assets/pbr/lut.png',
});
const envDiffuseTexture = TextureLoader.load(gl, {
	src: '/node/ogl/assets/sunset-diffuse-RGBM.png',
});
const envSpecularTexture = TextureLoader.load(gl, {
	src: '/node/ogl/assets/sunset-specular-RGBM.png',
});
...
const loadInitial=async()=>{//«
	gltf = await GLTFLoader.load(gl, `/node/ogl/assets/gltf/hershel.glb`);
//	gltf = await GLTFLoader.load(gl, `/assets/models/low_poly_char.glb`);
	addGLTF(gltf);
}//»
const handlers=()=>{//«
